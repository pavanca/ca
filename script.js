...
function renderFinalHero() {
  ...
  renderHero();
  const questions = hero.questions || [hero.title];
  const title = $("hero-title");
  if (!title) return;
  let index = 0;
  title.textContent = questions[0];
  title.classList.add("hero-question-active");
  if (window.matchMedia("(prefers-reduced-motion: reduce)").matches || questions.length < 2) return;
  setInterval(() => {
    title.classList.remove("hero-question-active");
    title.classList.add("hero-question-leaving");
    setTimeout(() => {
      index = (index + 1) % questions.length;
      title.textContent = questions[index];
      title.classList.remove("hero-question-leaving");
      title.classList.add("hero-question-active");
    }, 280);
  }, 2800);
}

function renderFinalServices() {
  const section = SITE_CONFIG.sections.services, services = SITE_CONFIG.services;
  setText("services-eyebrow", section.eyebrow); setText("services-title", section.title); setText("services-description", "Tell us what you need help with.");
  const grid = $("services-grid"); if (!grid) return;
  grid.innerHTML = services.map((s,i)=>`<article class="service-accordion" data-open="false">
    <button class="service-accordion-trigger" type="button" aria-expanded="false">
      <span class="service-number">${String(i+1).padStart(2,"0")}</span><span class="service-accordion-title">${escapeHTML(s.title)}</span><span class="service-accordion-icon">+</span>
    </button>
    <div class="service-accordion-panel"><div class="service-accordion-inner">
      <ul>${s.details.map(d=>`<li>${escapeHTML(d)}</li>`).join("")}</ul>
      <a class="service-action" href="${escapeHTML(getServiceWhatsAppUrl(s.id))}" target="_blank" rel="noopener">Ask us about this <i data-lucide="arrow-up-right"></i></a>
    </div></div></article>`).join("");
  grid.querySelectorAll(".service-accordion-trigger").forEach(btn=>btn.addEventListener("click",()=>{
    const item=btn.closest(".service-accordion"), open=item.dataset.open!=="true";
    grid.querySelectorAll(".service-accordion").forEach(x=>{x.dataset.open="false";x.querySelector("button").setAttribute("aria-expanded","false");x.querySelector(".service-accordion-icon").textContent="+"});
    if(open){item.dataset.open="true";btn.setAttribute("aria-expanded","true");item.querySelector(".service-accordion-icon").textContent="−";}
  }));
  setVisible("services-not-sure", false);
}

function renderSimpleLists() {
  renderAudiences(); renderIndustries();
  const a=$("audience-grid"), i=$("industries-grid");
  if(a) a.innerHTML=(SITE_CONFIG.audiences||[]).map(x=>`<div class="simple-list-item">${escapeHTML(x.title)}</div>`).join("");
  if(i) i.innerHTML=(SITE_CONFIG.industries||[]).map(x=>`<div class="simple-list-item">${escapeHTML(x.title)}</div>`).join("");
}

function renderAdvantage() {
  const c=SITE_CONFIG.sections.advantage; if(!c) return;
  setText("advantage-eyebrow",c.eyebrow); setText("advantage-title",c.title); setText("advantage-description",c.description); setText("advantage-closing",c.closing);
  const j=$("advantage-journey"), s=$("advantage-strengths");
  if(j) j.innerHTML=c.journey.map((x,i)=>`<div class="advantage-stage"><span class="advantage-number">${x.number}</span><span class="mini-label">${escapeHTML(x.label)}</span><strong>${escapeHTML(x.company)}</strong><span>${escapeHTML(x.role)}</span>${i<c.journey.length-1?'<span class="advantage-arrow" aria-hidden="true">→</span>':''}</div>`).join("");
  if(s) s.innerHTML=c.strengths.map(x=>`<article class="advantage-strength"><h3>${escapeHTML(x.title)}</h3><p>${escapeHTML(x.text)}</p></article>`).join("");
}

/* Re-render approved final sections after base initialization. */
document.addEventListener("DOMContentLoaded",()=>{
  setTimeout(()=>{ renderFinalHero(); renderFinalServices(); renderSimpleLists(); renderAdvantage(); refreshIcons(); },0);
});
...
function renderFinalFooter() {
  setText("footer-copyright", SITE_CONFIG.footer?.copyright || "© 2026 Pavan Reddy & Co. All rights reserved.");
}

function cleanupFinalSections() {
  ["digital-office", "how-we-work", "start-here", "quick-alert", "our-story"]
    .forEach(id => $(id)?.remove());

  setVisible("services-not-sure", false);

  const finalMessage = $("contact-final-message");
...
document.addEventListener("DOMContentLoaded", () => {
  window.setTimeout(cleanupFinalSections, 10);
});
